...
for /f "delims=" %%A in ('powershell -Command "(Get-Content .env | Select-String 'NGROK_AUTH' | ForEach-Object { ($_ -split '=')[1] }) -replace '^\s+|\s+$',''"') do set NGROK_AUTH=%%A
for /f "delims=" %%A in ('powershell -Command "(Get-Content .env | Select-String 'NGROK_DOMAIN' | ForEach-Object { ($_ -split '=')[1] }) -replace '^\s+|\s+$',''"') do set NGROK_DOMAIN=%%A
